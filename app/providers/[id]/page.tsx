...
import { Star, MapPin, Clock, ChevronLeft, Loader2, CheckCircle2, X } from "lucide-react";
...
export default function ProviderDetailPage() {
  ...
  const [step, setStep] = useState<"info" | "booking" | "confirm">("info");
  ...
  useEffect(() => {
    if (selectedDate && provider) {
      providerService.getAvailableSlots(provider.id, selectedDate)
  ...
  return (
    <div className="min-h-dvh pb-28 page-enter">
      {/* Back button */}
      <button
        onClick={() => step === "info" ? router.back() : setStep("info")}
        className="fixed top-12 left-5 z-30 w-10 h-10 rounded-xl bg-[#1a1210]/80 backdrop-blur-sm border border-orange-900/20 flex items-center justify-center text-dark-300 hover:text-dark-100 transition-colors"
      >
        <ChevronLeft className="w-5 h-5" />
      </button>

      {/* Hero */}
      <div className="relative h-48 bg-gradient-to-br from-orange-900/30 to-[#1a1210] flex items-end px-5 pb-5">
        <div className="absolute inset-0 opacity-10">
          <div className="w-full h-full" style={{
            background: `radial-gradient(circle at 70% 30%, ${category?.color}40, transparent 70%)`
          }} />
        </div>
        <div className="absolute right-5 top-1/2 -translate-y-1/2 text-7xl opacity-20">
          {category?.icon}
        </div>
      ...
      <div className="px-5 mt-5 space-y-5">
        ...
        <div className="card p-4">
          ...
          <div className="flex items-center gap-2 mt-3 text-xs text-dark-500 font-body">
            <MapPin className="w-3.5 h-3.5 text-dark-600" />
            {provider.address}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {provider.services.map((svc) => (
              <button
                ...
                className={clsx(
                  "w-full card p-4 flex items-center justify-between text-left transition-all duration-200",
                  selectedService?.id === svc.id
                    ? "border-orange-500/40 bg-orange-500/8"
                    : "hover:border-orange-900/40"
                )}
              >
                <div>
        ...
        {step === "booking" && selectedService && (
          <div className="card p-5 space-y-5 border-orange-500/25">
            <div className="flex items-center justify-between">
              <h2 className="section-title">Escolher data e hora</h2>
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {dates.map((date) => {
                  ...
                  return (
                    <button
                      key={val}
                      onClick={() => { setSelectedDate(val); setSelectedTime(""); }}
                      className={clsx(
                        "flex-shrink-0 flex flex-col items-center p-3 rounded-xl border transition-all w-16 text-center",
                        selectedDate === val
                          ? "bg-orange-500 border-orange-500 text-white"
                          : "bg-[#1a1210] border-orange-900/20 text-dark-400 hover:border-orange-900/40"
                      )}
                    >
            ...
            <div>
              ...
              <textarea
                className="input-field resize-none h-20 pt-3"
                placeholder="Ex: prefiro mais curto nas laterais..."
                value={notes}
