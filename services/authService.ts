...
export const authService = {
  ...
  async forgotPassword(email: string): Promise<void> {
    if (USE_MOCK) {
      ...
      if (!email.includes("@")) {
        throw { response: { data: { message: "Email inválido" }, status: 422 } };
      }
      return;
    }
  ...
  async verifyResetCode(email: string, code: string): Promise<void> {
    if (USE_MOCK) {
      await mockDelay(800);
      // Mock aceita qualquer código de 6 dígitos exceto "000000"
      if (code === "000000") {
        throw { response: { data: { message: "Código inválido ou expirado" }, status: 422 } };
      }
      return;
    }
